...
export async function generateEnterpriseReport({ overview, appointments, doctors, governance, days }) {
  ...
  const ratingBands = [
    { label: '⭐⭐⭐⭐⭐  Excellent  (5.0)', from: 4.5, color: C.accent  },
    { label: '⭐⭐⭐⭐    Good       (4.0–4.4)', from: 4.0, color: [34,197,94]  },
    { label: '⭐⭐⭐      Average    (3.0–3.9)', from: 3.0, color: C.warning },
    { label: '⭐⭐        Poor       (2.0–2.9)', from: 2.0, color: [249,115,22]  },
    { label: '⭐          Critical   (<2.0)',    from: 0,   color: C.danger  },
  ];
  ...
  autoTable(doc, {
    ...
    body: ratingBands.map(b => [
      b.label,
      Number(rev.average_rating) >= b.from ? '● Current system is at or above this level' : '',
    ]),
