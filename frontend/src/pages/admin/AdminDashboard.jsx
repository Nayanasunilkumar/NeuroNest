...
const AdminDashboard = () => {
    ...
    const getStatTone = (id) => {
        switch (id) {
            case 'patients':
                return { bg: 'var(--admin-tone-patients-bg)', fg: 'var(--admin-tone-patients-fg)' };
            case 'doctors':
                return { bg: 'var(--admin-tone-doctors-bg)', fg: 'var(--admin-tone-doctors-fg)' };
            case 'load':
                return { bg: 'var(--admin-tone-load-bg)', fg: 'var(--admin-tone-load-fg)' };
            case 'revenue':
                return { bg: 'var(--admin-tone-revenue-bg)', fg: 'var(--admin-tone-revenue-fg)' };
            default:
                return { bg: 'var(--nn-surface-secondary)', fg: 'var(--nn-primary)' };
        }
    };
    ...
    return (
        <div className="container-fluid py-4 min-vh-100 px-lg-4 admin-dashboard-shell">
            {/* Header */}
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-5">
                <div>
                    ...
                    <h1 className="h3 fw-black mb-0" style={{ color: 'var(--nn-text-main)' }}>System Overview</h1>
                </div>
                <button className="nn-btn d-flex align-items-center gap-2 border-0 admin-broadcast-btn">
            ...
            <div className="row g-4 mb-5">
                {data.stats.map((stat, index) => (
                    <div key={index} className="col-12 col-sm-6 col-xl-3">
                        <div className="card border-0 shadow-sm rounded-4 h-100 transition-all hover-translate-y admin-stat-card">
                            <div className="card-body p-4">
                                <div className="d-flex justify-content-between align-items-start mb-3">
                                    <div className="p-3 rounded-4" style={{ background: getStatTone(stat.id).bg, color: getStatTone(stat.id).fg }}>
                                        {getStatIcon(stat.id)}
                                    </div>
                                    <span
                                        className={`nn-badge ${stat.trend?.startsWith('+') ? 'nn-badge-success' : stat.trend === 'Stable' ? 'admin-stable-badge' : 'nn-badge-danger'}`}
                                    >
                                ...
                                <div className="h2 fw-black mb-0" style={{ color: 'var(--nn-text-main)' }}>{stat.value}</div>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="row g-4 mb-5">
                ...
                <div className="col-12 col-lg-8">
                    <div className="card border-0 shadow-sm rounded-4 h-100 admin-panel-card">
                        <div className="card-header bg-transparent border-0 p-4 pb-0">
                            <div className="d-flex justify-content-between align-items-center">
                                <h2 className="h6 fw-bolder mb-0 text-uppercase" style={{ letterSpacing: '1px', color: 'var(--nn-text-main)' }}>Engagement Metrics</h2>
                                <div className="d-flex gap-3">
                                    <div className="d-flex align-items-center gap-2 small fw-bold text-secondary">
                                        <div className="rounded-circle" style={{ width: '8px', height: '8px', background: 'var(--nn-primary)' }}></div> Intake
                                    </div>
                                    <div className="d-flex align-items-center gap-2 small fw-bold text-secondary">
                                        <div className="rounded-circle" style={{ width: '8px', height: '8px', background: 'var(--nn-secondary)' }}></div> Outflow
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                {data.chartData.map((d, i) => (
                                    <div key={i} className="d-flex flex-column align-items-center gap-3 flex-grow-1">
                                        <div className="d-flex gap-1 align-items-end w-100 justify-content-center" style={{ height: '200px' }}>
                                            <div className="rounded-top shadow-sm transition-all" style={{ width: '12px', height: `${d.p}%`, background: 'linear-gradient(to top, var(--nn-primary-hover), var(--nn-primary))' }} title={`Intake: ${d.p}`}></div>
                                            <div className="rounded-top shadow-sm transition-all" style={{ width: '12px', height: `${d.s}%`, background: 'linear-gradient(to top, var(--nn-secondary), color-mix(in srgb, var(--nn-secondary) 62%, white))' }} title={`Outflow: ${d.s}`}></div>
                                        </div>
                ...
                <div className="col-12 col-lg-4">
                    <div className="card border-0 shadow-sm rounded-4 h-100 admin-panel-card">
                        <div className="card-header bg-transparent border-0 p-4 pb-0">
                            <h2 className="h6 fw-bolder mb-0 text-uppercase" style={{ letterSpacing: '1px', color: 'var(--nn-text-main)' }}>Active Tasks</h2>
                        </div>
                        <div className="card-body p-4 pt-4">
                            <div className="d-flex flex-column gap-3">
                                {data.tasks.map((task, i) => (
                                    <div key={i} className="p-3 rounded-4 border-start border-4 shadow-xs transition-all hover-translate-x admin-task-item">
                                        <div className="d-flex justify-content-between align-items-start mb-1">
                                            <h4 className="h6 fw-bolder mb-0" style={{ fontSize: '0.9rem', color: 'var(--nn-text-main)' }}>{task.title}</h4>
            ...
            <div className="row g-4">
                <div className="col-12 col-xl-8">
                    <div className="mb-4">
                        <h2 className="h5 fw-black mb-4" style={{ color: 'var(--nn-text-main)' }}>Operations Nexus</h2>
                        <div className="row g-3">
                            {modules.map((mod, index) => (
                                <div key={index} className="col-12 col-md-6">
                                    <Link to={mod.path} className="card border-0 shadow-sm rounded-4 text-decoration-none h-100 transition-all hover-translate-y admin-module-card">
                                        <div className="card-body p-4 d-flex align-items-center gap-4">
                                            ...
                                            <div className="min-w-0">
                                                <h3 className="h6 fw-bolder mb-1" style={{ color: 'var(--nn-text-main)' }}>{mod.title}</h3>
                ...
                <div className="col-12 col-xl-4">
                    <h2 className="h5 fw-black mb-4" style={{ color: 'var(--nn-text-main)' }}>System Logs</h2>
                    <div className="card border-0 shadow-sm rounded-4 admin-panel-card">
                        <div className="card-body p-4 d-flex flex-column gap-3">
                            <div className="activity-list overflow-y-auto custom-scrollbar pe-2" style={{ maxHeight: '380px' }}>
                                {data.activities.map((act, index) => (
                                    <div key={index} className="border-bottom pb-3 mb-3 last-mb-0 last-pb-0" style={{ borderColor: 'var(--nn-divider)' }}>
                                        ...
                                        <p className={`small mb-0 fw-medium ${act.type === 'error' ? 'text-danger' : ''}`} style={{ fontSize: '0.8rem', lineHeight: '1.4', color: act.type === 'error' ? undefined : 'var(--nn-text-main)' }}>
                                            {act.text}
                                        </p>
                                    </div>
                                ))}
            ...
            <style>{`
                ...
                .admin-task-item {
                    background: color-mix(in srgb, var(--nn-surface-secondary) 85%, transparent);
                    border-color: var(--nn-primary) !important;
                }
                .admin-stable-badge {
                    background: color-mix(in srgb, var(--nn-surface-secondary) 90%, transparent);
                ...
                .custom-scrollbar::-webkit-scrollbar-thumb { background: var(--nn-border-strong); border-radius: 10px; }
            `}</style>
        </div>
    );
};
